...
class SignalProcessor:
    ...
    def compute_psd(self, samples: np.ndarray, method: str = 'welch') -> Tuple[np.ndarray, np.ndarray]:
        """
    ...
    def find_peaks(self, spectrum: np.ndarray, 
                   height: float = -80, 
                   distance: int = 10,
                   prominence: float = 3) -> Tuple[np.ndarray, Dict[str, Any]]:
        """
    ...
    def detect_signals(self, spectrum: np.ndarray, 
                      center_freq: float = 0,
                      threshold: float = -70) -> List[Dict[str, Any]]:
        """
    ...
    def _estimate_bandwidth(self, spectrum: np.ndarray, peak_idx: int, 
    ...
    def _estimate_snr(self, spectrum: np.ndarray, peak_idx: int, 
        ...
        noise_samples = np.concatenate([
            spectrum[start_idx:peak_idx-5],
            spectrum[peak_idx+5:end_idx]
        ])
